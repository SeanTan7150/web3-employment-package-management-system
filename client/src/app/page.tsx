...
export default function Home() {
  ...
  const { getEmployer, activeAccount } = useContractContext();
  ...
  const handleLogin = async () => {
    ...
    setIsLoading(true);
    const payload = await generatePayload({ address: activeAccount.address });
    ...
    if (!signatureResult) {
      setIsLoading(false);
      console.log("User rejected to sign SIWE");
      return;
    }

    await login(signatureResult);

    // if user not exist in db, create new user profile in db
    const user: UserResponse = await readUserByAddress(activeAccount.address);

    if (!user.fetched) {
      console.log("New user connected, preparing user profile");
      router.push("/register");
      // await createUser(activeAccount.address, "", "0xnew", "user");
    } else if (!user.user.is_activated) {
      alert(
        "Looks like your account has not activated by your employer yet..."
      );
    } else {
      router.push("/ethiring");
    }
    setIsLoading(false);
  };
      ...
      <div className="mt-20">
        <div className="mt-64 relative z-10 flex flex-col justify-center items-center h-full text-center">
          ...
          <p className="mb-6 text-lg font-normal text-gray-500 lg:text-xl sm:px-16 xl:px-48">
            ...
            system, designed to make managing employment packages seamless and
            efficient.
          </p>

          {activeAccount ? (
            // <button
            //   className="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:from-white hover:to-white border-solid border-white border-2 hover:border-blue-400 hover:text-blue-400 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-lg px-6 text-center me-2 h-12"
            //   disabled={!activeAccount || isLoading}
            //   onClick={handleLogin}
            // >
            //   {!isLoading ? `Sign in to Ethiring` : `Wait a sec...`}
            // </button>
            <button
              type="button"
              className="text-gray-900 bg-white hover:bg-blue-600 hover:text-white border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-base px-5 py-2 text-center inline-flex items-center me-2 mb-2 disabled:cursor-not-allowed disabled:bg-blue-600 disabled:text-white"
              disabled={!activeAccount || isLoading}
              onClick={handleLogin}
            >
              <svg
